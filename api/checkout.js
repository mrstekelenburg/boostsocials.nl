...
// Likes en views worden op een specifieke post geleverd, dus daar is een
// postlink voor nodig. Bereik/volgers en abonnees gelden voor het profiel.
const heeftPostNodig = (id) => /-(l|vw)-/.test(id || '');
...
module.exports = async (req, res) => {
  ...
  const needsLink = heeftPostNodig(productId) || heeftPostNodig(upsellId);
  ...
  try {
    const payment = await mollie.payments.create({
      ...
      redirectUrl: `${origin}/succes.html?username=${encodeURIComponent(username)}&pakket=${encodeURIComponent(omschrijving)}&value=${toAmount(cents)}&order=${orderRef}${needsLink ? '&link=1' : ''}`,
    ...
    // Het payment-id bestaat pas na het aanmaken, dus de redirect wordt hier
    // bijgewerkt zodat succes.html de bestelling kan verifieren bij Mollie.
    // Mislukt dit, dan gaat de bestelling gewoon door zonder linkformulier.
    if (needsLink) {
      try {
        await mollie.payments.update(payment.id, {
          redirectUrl: `${payment.redirectUrl}&pid=${payment.id}`,
        });
      } catch (e) {
        console.error('Redirect bijwerken mislukt:', e.message);
      }
    }
